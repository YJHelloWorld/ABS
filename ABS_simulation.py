...
if __name__ == '__main__':
    start = time.time()
    Nf = 7; L =2000; Q = 50;nside= 1024
    #simulation(nside)
    ###false_number = len(mask[mask==False])
    ##fr = 1 #float(false_number)/(12*nside**2)
    total = np.load('../simulation/Nside = 1024/total_map.npy')
    ...
    cmb = np.load('../simulation/Nside = 1024/cmb_map.npy')
    ##total_masked = Mask(total)
    ##cmb_masked = Mask(cmb)
    #CL = hp.anafast((cmb)[0][0],lmax = L,gal_cut=10)    #cmb_MASKED,gal_cut=10   
    #np.save('../simulation/Nside = 1024/9_fre/cmb_power_spectrum.npy', CL)    
    #exit()
    
    #white_noise
    for n in range(0,20):
        #white_noise = np.load('/home/yao/Desktop/EM-smica/ABS/simulation/Nside = 1024/noise_map_NO%s.npy'%(n+2))
        white_noise = np.ones_like(total); cl_noise = np.ones((Nf,L+1));  Nl = (0.066**2,0.065**2,0.063**2,0.028**2,0.015**2,0.023**2,0.068**2)#(0.066**2,0.065**2,0.063**2,0.028**2,0.015**2,0.023**2,0.068**2,0.62**2, 28.4**2); 
        ...
        for i in range(Nf):
            ...
            noise_power_spectrum = hp.anafast((white_noise)[i][0], lmax = L,gal_cut= 40)
        ...
        np.save('/home/yao/Desktop/EM-smica/ABS/simulation/Nside = 1024/null_test/b_40/noise_power_spectrum_NO%s.npy'%(n+1), Nl_matrix)        
        
        #white_noise_masked = Mask(white_noise)
        #total_masked = Mask(total - cmb + white_noise)
    
        total_masked_power_spectrum = power_spectrum(total+white_noise-cmb, 40)
        ###total_power_spectrum = power_spectrum(total + white_noise)         
        np.save('../simulation/Nside = 1024/null_test/b_40/total_power_spectrum_NO%s.npy'%(n+1), total_masked_power_spectrum) 
